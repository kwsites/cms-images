(function () {
         ...
         self.play = function ($widget, data, options) {
            ...
            switch (renderer) {
               ...
               case 'gallery':
                  ...
                  container.hasClass('mobile-carousel') && mobileCarousel(container)
                  break;
            }
   ...
   function mobileCarousel ($gallery) {
      if (!isMobileLayout()) {
         return;
      }

      function current () {
         return $gallery.find('.' + THUMB_CLASS).eq(0);
      }

      function goNext () {
         var $current = current(0);

         var $departing = $current
            .clone()
            .addClass('fade-out-slide')
            .prependTo($gallery)
            .fadeOut({
               duration: 1500,
               done: function () {
                  $departing.remove();
                  isMobileLayout() && setTimeout(goNext, 2500);
               }
            });
      ...
      function isMobileLayout () {
         return typeof window.matchMedia === 'function' && matchMedia('(max-width: 414px)').matches;
      }
   }
   ...
   function galleryPlayer (container, slideParent) {
      ...
      container.on('click', '[data-action="back"]', function () {
         return showNext(-1)
      });
      ...
      function showImage (index) {
         ...
         img.onload = function () {
            ...
            setTimeout(function () {
               image.removeClass('cms-gallery-next-image')
            }, 5);
         };
   ...
   function defaultPlayer (container, slideParent, interval) {
      ...
      container.find('[data-action="next"]').on('click', function () {
         goNext(slideParent);
         schedule();
      });
